...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (selectedCat) {
        ...
        if (catResult.meals) {
            if (!data) // no data yet, initialize
                data = catResult.meals;
            else // filter current data
                data = data.filter(meal => catResult.meals.some(catMeal => catMeal.idMeal === meal.idMeal));
        }
    }
    // Apply area filter
    if (selectedArea) {
        ...
        if (areaResult.meals) {
            if (!data) // no data yet, initialize
                data = areaResult.meals;
            else // filter current data
                data = data.filter(meal => areaResult.meals.some(areaMeal => areaMeal.idMeal === meal.idMeal));
        }
    }
    ...
    if (Array.isArray(filteredIngredients) && filteredIngredients.length > 0) {
        ...
        if (ingResult.meals) {
            if (!data) // no data yet, initialize
                data = ingResult.meals;
            else // filter current data
                data = data.filter(meal => ingResult.meals.some(ingMeal => ingMeal.idMeal === meal.idMeal));
        }
    }
