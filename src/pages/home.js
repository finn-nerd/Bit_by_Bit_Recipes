...
function Home({ isLoggedIn }) {
    ...
    const toggleSavedMeal = (meal) => {
        ...
        setSavedMeals((prev) => (isNowSaved ? [...prev, meal.idMeal] : prev.filter((id) => id !== meal.idMeal)));
        // Update DB
            ...
            <div className="App flex flex-col min-h-screen">
                ...
                <div className="py-1 top-0 left-0 w-full bg-gradient-to-b from-[#F18D5E] to-[#EF6F34] z-10">
                    <div className="flex flex-row gap-3 items-center my-5">
                        ...
                        {/* Random Recipe */}
                        <button className="cursor-pointer p-2 hover:scale-105 transition-transform ml-auto btn-active no-shadow" onClick={handleRandom}>
                            <svg
                        ...
                        {/* Refresh Recipes */}
                        <button className="cursor-pointer p-2 ml-auto flex items-center justify-center no-shadow" onClick={() => fetchMeals()}>
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 24 24"
                                className="w-12 h-12 text-white"
                ...
                <div className="flex flex-row flex-grow">
                    ...
                    <div className="py-1 overflow-y-auto scrollbar-hide w-[13%] bg-gradient-to-b from-[#EEAE36] to-[#E97832] z-1 flex flex-col items-center pt-5">
                        ...
                        <Listbox as="div" value={selectedCat} onChange={handleSelectCat}>
                            {/* Dropdown button */}
                            ...
                            <Listbox.Options>
                                {categories.map((category, index) => (
                                    <Listbox.Option key={index} value={category} className={({ active }) => `cursor-pointer select-none p-2 ${active ? "bg-[#EF6F34] text-black" : ""}`}>
                                        {category}
                                    </Listbox.Option>
                                ))}
                            </Listbox.Options>
                        ...
                        <Listbox as="div" value={selectedArea} onChange={handleSelectArea}>
                            {/* Dropdown button */}
                            <Listbox.Button className="w-full cursor-pointer text-xl sm:text-2xl xl:text-[22px] text-black py-3 focus:outline-none opacity-[60%] font-['Jersey_10']">
                                {selectedArea || "Any"}
                            </Listbox.Button>
                            ...
                            <Listbox.Options>
                                {areas.map((area, index) => (
                                    <Listbox.Option key={index} value={area} className={({ active }) => `cursor-pointer select-none p-2 ${active ? "bg-[#EF6F34] text-black" : ""}`}>
                                        {area}
                                    </Listbox.Option>
                                ))}
                            </Listbox.Options>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1" style={{ height: "65%" }}>
                                                <img src={meal.strMealThumb} alt={meal.strMeal} className="w-full h-full object-cover rounded-t-[12px]" />
                                            </div>
